...
include_once($shared_tables_include_path . "/sites/all/shared_resources/bin/shared_tables.php");        
include_once($shared_tables_include_path . "/sites/all/shared_resources/functions/functions_credential.php");

$hostname=gethostname();
if (function_exists('drush_get_context')) { // for drush  
  $is_server=strtolower(substr($hostname,-5, -1)) == "ws00" ? TRUE : FALSE;
  $dbname=get_db_name(end(explode('://', drush_get_context('DRUSH_URI'))));
} else { // for php web
  $is_server=(substr($_SERVER['SERVER_ADDR'],0,7) == "149.171") ? TRUE : FALSE;
  $dbname=get_db_name($_SERVER['SERVER_NAME']);
}
$password=get_credential($hostname);
$dbserver=$is_server ? str_replace("ws00", "db00", $hostname) : "localhost";

// we don't need to shared tables for profiles db
if ($dbname=="profiles") $shared_tables="";
 ...
 *
 */
$drupal_hash_salt = 'BlMg_IzHikFoAKgapIM9F8bO-io4gVwjloZgpQ9ktZ8';
